        ...
        <div class="row">
            @foreach ($sanPhams as $sanPham)
                <div class="col-md-4 mb-4">
                    <div class="card">
                        <img src="{{ asset('storage/' . $sanPham->pdpHinhAnh) }}" class="card-img-top product-img" alt="{{ $sanPham->pdpTenSanPham }}">
                        <div class="card-body">
                            <h5 class="card-title">{{ $sanPham->pdpTenSanPham }}</h5>
                            <p class="card-text"><strong>{{ number_format($sanPham->pdpDonGia, 0, ',', '.') }} VND</strong></p>
                            <p class="card-text"><small class="text-muted">Số lượng còn lại: {{ $sanPham->pdpSoLuong }}</small></p>
                            
                            <!-- Nút xem chi tiết -->
                            <a href="{{ route('pdpuser.show', $sanPham->id) }}" class="btn btn-primary btn-sm flex " style="justify-content: center">Xem Chi Tiết</a>

                            <!-- Nút Mua và icon giỏ hàng -->
                            <a href="{{ route('pdpuser.pdpthanhtoan', ['product_id' => $sanPham->id]) }}" class="btn btn-primary btn-sm">
                                <i class="fa fa-shopping-cart"></i> Mua
                            </a>
                            

                            <!-- Thêm vào giỏ hàng -->
                            <button type="button" class="btn btn-warning btn-sm add-to-cart-btn" data-id="{{ $sanPham->id }}" data-name="{{ $sanPham->pdpTenSanPham }}">
