            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach($products as $sanpham)
                    <div class="bg-white shadow-lg rounded-lg overflow-hidden transform transition-all hover:scale-105 hover:shadow-xl">
                        <!-- Image -->
                        <img src="{{ asset('storage/' . $sanpham->pdpHinhAnh) }}" class="w-full h-56 object-cover" alt="{{ $sanpham->pdpTenSanPham }}">

                        <div class="p-4">
                            <h5 class="text-lg font-semibold text-gray-800">{{ $sanpham->pdpTenSanPham }}</h5>
                            <p class="text-red-500 font-bold mt-2">{{ number_format($sanpham->pdpDonGia, 0, ',', '.') }} VND</p>
                            <p class="text-sm text-gray-600 mt-2"><small>Số lượng còn lại: {{ $sanpham->pdpSoLuong }}</small></p>

                            <div class="mt-4">
                                <!-- Xem chi tiết (on the first row) -->
                                <div class="mb-2">
                                    <a href="{{ route('pdpuser.show', $sanpham->id) }}"style="text-align:center" class="bg-blue-500 text-white py-2 px-4 rounded-md text-sm hover:bg-blue-600 transition duration-300 w-full block">
                                        Xem Chi Tiết
                                    </a>
                                </div>
                                ...
                                <div class="flex gap-4">
                                    ...
                                    <a href="javascript:void(0);"   
                                       ...
                                       onclick="return confirm('Bạn muốn mua {{ $sanpham->pdpTenSanPham }} này không ?');" 
                                       title="Mua">
                                    ...
                                    <button type="button" class="bg-yellow-500 text-white py-2 px-4 rounded-md text-sm hover:bg-yellow-600 transition duration-300 add-to-cart-btn" 
                                            data-id="{{ $sanpham->id }}" data-name="{{ $sanpham->pdpTenSanPham }}">
